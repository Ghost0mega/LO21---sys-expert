...
static void strlist_add_sorted_unique(StrNode **h, const char *s) {
  ...
  while (cur && strcmp(cur->s, s) < 0) {
    prev = cur;
    cur = cur->next;
  }
  if (cur && strcmp(cur->s, s) == 0)
    return;
...
static void strlist_free(StrNode *h) {
  while (h) {
    StrNode *n = h->next;
    free(h->s);
    free(h);
...
static int strlist_len(StrNode *h) {
  int c = 0;
  for (; h; h = h->next)
    c++;
  return c;
}
static int strlist_maxlen(StrNode *h) {
  int m = 0;
  for (; h; h = h->next) {
    int L = (int)strlen(h->s);
    if (L > m)
      m = L;
  }
  return m;
}
static int strlist_index_of(StrNode *h, const char *s) {
  int i = 0;
  for (; h; h = h->next, ++i)
    if (strcmp(h->s, s) == 0)
      return i;
...
static const char *strlist_name_at(StrNode *h, int index) {
  int i = 0;
  for (; h; h = h->next, ++i)
    if (i == index)
      return h->s;
...
static void strlist_remove_at(StrNode **h, int index) {
  if (!h || !*h || index < 0)
    return;
  StrNode *prev = NULL, *cur = *h;
  int i = 0;
...
static void strlist_append_unique(StrNode **h, const char *s) {
  if (strlist_contains(*h, s))
    return;
  StrNode *n = (StrNode *)malloc(sizeof(StrNode));
  n->s = strdup(s);
  n->next = NULL;
...
static void build_variables(const BC *bc, StrNode **vars_out) {
  ...
  for (const ListRegleNode *rn = bc->regles.head; rn; rn = rn->next) {
    for (const ListPropositionNode *pn = rn->value.premises.head; pn;
         pn = pn->next) {
...
static void rebuild_facts(const BC *bc, StrNode *vars, int *base_states,
                          BaseFaits *out) {
  *out = facts_create();
  int idx = 0;
  for (StrNode *v = vars; v; v = v->next, ++idx) {
    if (base_states[idx])
      facts_add(out, proposition_make(v->s, 0));
  }
...
static void map_set_local(Map **pm, const char *name, int line) {
...
static int map_get_local(Map *m, const char *name, int *line) {
  for (Map *x = m; x; x = x->next)
...
static void draw_ascii(const BC *bc, StrNode *vars, const BaseFaits *facts,
                       int cursor_row, int y_offset) {
  ...
  int i = 0;
  for (StrNode *v = vars; v; v = v->next)
    var_names[i++] = v->s;

  // Precompute rules draw info (top,bottom,label line, premises lines)
  typedef struct PremI {
    int line;
    int neg;
    struct PremI *next;
  } PremI;
  typedef struct RuleI {
    const char *label;
    int top, bottom, label_line;
    PremI *p;
    struct RuleI *next;
  } RuleI;
  ...
  for (int k = 0; k < var_count; ++k)
    map_set_local(&m, var_names[k], 2 * k);

  for (const ListRegleNode *rn = bc->regles.head; rn; rn = rn->next) {
    const Regle *r = &rn->value;
    int top = 1e9, bottom = -1e9;
    PremI *p = NULL, *pt = NULL;
    for (const ListPropositionNode *pn = r->premises.head; pn; pn = pn->next) {
      int line = -1;
      if (!map_get_local(m, pn->value.name, &line))
        continue; // unknown yet
      PremI *pi = (PremI *)malloc(sizeof(PremI));
      pi->line = line;
      pi->neg = pn->value.negated ? 1 : 0;
      pi->next = NULL;
      if (!p)
        p = pt = pi;
      else {
    ...
    int label_line;
    if (!p) {
      label_line = total_lines;
      if (label_line >= total_lines)
        total_lines = label_line + 1;
    } else {
      label_line = (top + bottom) / 2;
      if ((label_line % 2) == 0)
        label_line = (label_line + 1 <= bottom)
                         ? label_line + 1
                         : ((top + 1 <= bottom) ? top + 1 : top);
      if ((label_line % 2) == 0) {
        label_line = bottom + 1;
        if (label_line >= total_lines)
          total_lines = label_line + 1;
    ...
    RuleI *ri = (RuleI *)malloc(sizeof(RuleI));
    ri->label = regle_has_conclusion(r) ? regle_get_conclusion(r).name : "";
    ri->top = top;
    ri->bottom = bottom;
    ri->label_line = label_line;
    ri->p = p;
    ri->next = NULL;
    if (!rules)
      rules = rtail = ri;
    ...
    if (regle_has_conclusion(r))
      map_set_local(&m, regle_get_conclusion(r).name, label_line);
  }
  ...
  for (int row = 0; row < total_lines; ++row) {
    ...
    if (row % 2 == 0) {
      ...
      if (vindex < var_count) {
        // Cursor
        if (vindex == cursor_row)
          addch('>');
        else
          addch(' ');
        addch(' ');
        // Name with highlight if fact true (use same highlighting as rules:
        // reverse)
        ...
        if ((int)strlen(nm) < left_name_w)
          for (int s = (int)strlen(nm); s < left_name_w; ++s)
            addch(' ');
        if (is_true)
          attroff(A_REVERSE);
      } else {
        // No variable for this even row; pad the left area
        for (int s = 0; s < left_w; ++s)
          addch(' ');
      }
    } else {
    ...
    for (RuleI *ri = rules; ri; ri = ri->next) {
      ...
      // connector cell: two chars width like print.c (leading space or '!') +
      // glyph
      int is_segment = (ri->top >= 0 && row >= ri->top && row <= ri->bottom);
      int is_prem = 0, neg = 0;
      if (is_segment) {
        for (PremI *pi = ri->p; pi; pi = pi->next)
          if (pi->line == row) {
            is_prem = 1;
            neg = pi->neg;
            break;
...
static int show_start_menu(void) {
  const char *items[3] = {"Base exemple", "Base vierge", "Quiter"};
  int n = 3;
  int sel = 0;
  while (1) {
    erase();
...
void run_ui(const BC *bc) {
  ...
  while (1) {
    ...
    // Prepare working knowledge base: example uses provided rules; personalized
    // starts empty
    BC local_bc;
    int use_local = (choice != 0);
    if (use_local) {
      local_bc = bc_create();
    }
    ...
    while (1) {
      ...
      attron(A_BOLD);
      mvprintw(0, 0,
               "↑/↓ move  •  SPACE toggle  •  i add input  •  d del input  •  "
               "a add rule  •  r del rule  •  q menu");
      attroff(A_BOLD);
      ...
      if (ch == 'q' || ch == 'Q')
        ...
        move(LINES - 1, 0);
        clrtoeol();
        mvprintw(LINES - 1, 0, "New input name: ");
        getnstr(buf, 127);
        noecho();
        curs_set(0);
        if (buf[0]) {
          if (!strlist_contains(vars, buf)) {
            ...
            // grow base_states
            base_states =
                (int *)realloc(base_states, sizeof(int) * (var_count + 1));
        ...
        if (var_count > 0) {
          ...
          if (name) {
            ...
            for (int k = selected; k < var_count - 1; ++k)
              base_states[k] = base_states[k + 1];
            var_count--;
            if (var_count == 0)
              selected = 0;
            else if (selected >= var_count)
              selected = var_count - 1;
            base_states = (int *)realloc(
                base_states, sizeof(int) * (var_count > 0 ? var_count : 1));
            facts_free(&facts);
            rebuild_facts(kb, vars, base_states, &facts);
          }
        }
      } else if (ch == 'a' || ch == 'A') {
        // Add rule: choose premises (vars + existing rules) with polarity, then
        // label Gather rule labels from current KB
        StrNode *rule_labels = NULL;
        for (const ListRegleNode *rn = kb->regles.head; rn; rn = rn->next) {
          if (regle_has_conclusion(&rn->value)) {
            const char *nm = regle_get_conclusion(&rn->value).name;
            // avoid duplicate if same as a variable name
            if (!strlist_contains(vars, nm))
              strlist_append_unique(&rule_labels, nm);
          }
        }
        int rule_count = strlist_len(rule_labels);
        int total_items = var_count + rule_count;
        int *sel_state =
            (int *)calloc((size_t)(total_items > 0 ? total_items : 1),
                          sizeof(int)); // 0 none, 1 pos, 2 neg
        int rr = 0;
        int add_ch;
        while (1) {
          erase();
          attron(A_BOLD);
          mvprintw(0, 0,
                   "Add rule: ↑/↓ move  •  space select +  •  ! toggle +/-  •  "
          ...
          refresh();
          add_ch = getch();
          if (add_ch == 'q' || add_ch == 'Q') {
            free(sel_state);
            sel_state = NULL;
            strlist_free(rule_labels);
            rule_labels = NULL;
            break;
          } else if (add_ch == KEY_UP) {
            if (rr > 0)
              rr--;
          } else if (add_ch == KEY_DOWN) {
            if (rr < total_items - 1)
              rr++;
          } else if (add_ch == ' ') {
            if (total_items > 0)
              sel_state[rr] = (sel_state[rr] == 0) ? 1 : 0;
          } else if (add_ch == '!') {
            if (total_items > 0) {
              if (sel_state[rr] == 0)
                sel_state[rr] = 2;
              else if (sel_state[rr] == 1)
                sel_state[rr] = 2;
              else
                sel_state[rr] = 1;
            }
            ...
            if (lab[0]) {
              ...
              for (int r = 0; r < var_count; ++r) {
                if (sel_state[r] == 1)
                  regle_add_premise(
                      &nr, proposition_make(strlist_name_at(vars, r), 0));
                else if (sel_state[r] == 2)
                  regle_add_premise(
                      &nr, proposition_make(strlist_name_at(vars, r), 1));
              }
              ...
              for (int r = 0; r < rule_count; ++r) {
                ...
                if (sel_state[idx] == 1)
                  regle_add_premise(&nr, proposition_make(nm, 0));
                else if (sel_state[idx] == 2)
                  regle_add_premise(&nr, proposition_make(nm, 1));
              }
        ...
        if (rc > 0) {
          ...
          for (const ListRegleNode *rn = kb->regles.head; rn; rn = rn->next)
            if (regle_has_conclusion(&rn->value)) {
              labels[idx++] = regle_get_conclusion(&rn->value).name;
            }
          int rrsel = 0;
          int rch;
          while (1) {
            ...
            attron(A_BOLD);
            mvprintw(0, 0,
                     "Remove rule: ↑/↓ move  •  ENTER delete  •  q cancel");
            attroff(A_BOLD);
            for (int i = 0; i < rc; ++i) {
              move(2 + i, 0);
              clrtoeol();
            ...
            if (rch == 'q' || rch == 'Q')
              ...
              if (rrsel > 0)
                rrsel--;
            } else if (rch == KEY_DOWN) {
              if (rrsel < rc - 1)
                rrsel++;
            } else if (rch == '\n' || rch == '\r' || rch == KEY_ENTER) {
              bc_remove_rule_by_label((BC *)kb, labels[rrsel]);
              facts_free(&facts);
              rebuild_facts(kb, vars, base_states, &facts);
              break;
            }
