...
void bc_print_ascii(const BC *bc) {
  ...
  for (const ListRegleNode *n = bc->regles.head; n; n = n->next) {
    ...
    if (!prem) {
      // No mappable premises; still register rule name with label under last
      // line
      ...
      if (regle_has_conclusion(r))
        name_map_set(&name_to_line, regle_get_conclusion(r).name, label_line);
      continue;
    }
    ...
    if ((label_line % 2) == 0) {
      if (label_line + 1 <= bottom)
        label_line += 1;
      else if (label_line - 1 >= top)
        label_line -= 1;
    }
    if (label_line < top || label_line > bottom)
      label_line = top + 1 <= bottom ? top + 1 : top; // fallback
    if ((label_line % 2) == 0) {
  ...
  int use_utf8 = 1;
#if defined(__STDC_ISO_10646__) || defined(__STDC_UTF_16__) ||                 \
    defined(__STDC_UTF_32__)
  use_utf8 = 1;
#endif
  ...
  for (RuleDraw *rd = rules; rd; rd = rd->next) {
    ...
    for (int l = 0; l < total_lines; ++l) {
      if (rd->top >= 0 && l >= rd->top && l <= rd->bottom) {
        int is_prem = 0, neg = 0;
        if (premises_contains_line(rd->premises, l, &neg))
          is_prem = 1;
