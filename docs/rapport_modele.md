...
Pour valider l'implémentation de chaque type abstrait et du moteur d'inférence, une suite de tests unitaires autonomes a été développée. Chaque test vérifie :
- **Création et initialisation** : les structures sont correctement allouées et initialisées.
- **Opérations élémentaires** : ajout, accès, suppression, tests de prédicats (appartenance, vacuité).
- **Invariants** : la structure respecte ses contraintes (ex. : cohérence de `size` avec le nombre réel de nœuds).
- **Cas limites** : structures vides, opérations sur NULL, mémoire correctement libérée sans fuites.

### 5.2. Suite de Tests Implémentée

Les tests sont organisés par type abstrait et localisés dans le répertoire `tests/` :

**`test_proposition.c`** :
- Création de propositions simples et négées.
- Comparaison d'égalité (même nom et même état de négation).
- Libération mémoire.

**`test_list_proposition.c`** :
- Création d'une liste vide.
- Insertion en queue (opération FIFO).
- Test d'appartenance (récursif et itératif).
- Suppression du premier élément et suppression totale par nom.
- Cas limite : listes vides, éléments non présents.

**`test_regle.c`** :
- Création d'une règle vide (prémisse vide, pas de conclusion).
- Ajout de propositions à la prémisse.
- Définition et récupération de conclusion.
- Test d'appartenance récursive dans la prémisse.
- Suppression de propositions de la prémisse.
- Accesseur de tête.

**`test_bc.c`** :
- Création d'une base de connaissances vide.
- Ajout de plusieurs règles en queue.
- Accès à la première règle.
- Suppression de règles par label de conclusion.
- Cas limite : tentative de suppression répétée.

**`test_inference.c`** :
- Construction d'une base de connaissances complexe.
- Exécution du moteur d'inférence sur des faits initiaux divers.
- Vérification des faits déduits attendus.
- Test de chaînage multi-étapes (règles en cascade).
- Validation du point fixe (arrêt de la propagation).

### 5.3. Résultats de la Validation

Tous les tests passent avec succès :
```
test_proposition: OK
test_list_proposition: OK
test_regle: OK
test_bc: OK
test_inference: OK
```
...
cd build
./test_proposition
./test_list_proposition
./test_regle
./test_bc
./test_inference
```

### 5.4. Couverture et Conformité

La suite de tests valide :
- ✅ Allocation et libération mémoire correctes (pas de fuite détectée).
- ✅ Invariants de structure respectés (cohérence des pointeurs, tailles).
- ✅ Opérations abstraites correctes et conformes aux spécifications du cours.
- ✅ Récursivité correcte (appartenance dans les listes).
- ✅ Moteur d'inférence converge au point fixe et produit les résultats attendus.
- ✅ Absence de crash, comportement défini sur tous les cas limites.
